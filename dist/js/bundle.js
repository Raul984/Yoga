...
function calc() {
  ...
  place.addEventListener('change', function () {
    if (restDays.value == '' || persons.value == '') {
      ...
      var a = total; //вводим техническую переменную чтобы правильно считался totalValue при нескольких нажатиях на разные place
...
function form() {
  ...
  sendForm(form);
  sendForm(formBottom); //если нужно отправить данные в текстовом формате (без Promise)
  // let message = {
  //     loading: 'Загрузка...',
  //     success: 'Спасибо! Скоро мы с вами свяжемся!',
  //     failure: 'Что-то пошло не так...'
...
function getModal(btnsSelector, containerSelector, closeSelector) {
  var btns = document.querySelectorAll(btnsSelector),
...
function timer() {
  ...
  function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        ...
        hours = Math.floor(t / (1000 * 60 * 60)); // Если нам нужны еще дни то прописываем:
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var calc = __webpack_require__(/*! ./parts/calc */ "./src/js/parts/calc.js"),
      ...
      getModal = __webpack_require__(/*! ./parts/modal */ "./src/js/parts/modal.js");
